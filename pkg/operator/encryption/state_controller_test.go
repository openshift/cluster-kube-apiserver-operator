...
func TestEncryptionStateController(t *testing.T) {
	scenarios := []struct {
		...
		// scenario 7
		// BUG: There are 4 keys (3 were migrated, 1 was observed as a read key), the encryption config with one migrated key (see scenario 6 - pruning) and one read key - both in the target ns and openshift-config-managed
		//      the test expects to see the updated encryption config with the read key observed as a write key
		//      instead, all 4 keys are in the updated encryption config and the read key wasn't promoted to be a write key
		{
			...
			expectedActions: []string{"list:pods:kms", "get:secrets:kms", "list:secrets:openshift-config-managed", "get:secrets:openshift-config-managed", "update:secrets:openshift-config-managed", "create:events:kms"},
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, destName string, expectedEncryptionCfg *apiserverconfigv1.EncryptionConfiguration) {
				// TODO: uncomment when the controller is fixed
				/*wasSecretValidated := false
				for _, action := range actions {
					if action.Matches("update", "secrets") {
						updateAction := action.(clientgotesting.UpdateAction)
		...
		//      as a result a new encryption config is created with a single read key - that effectively means that the encryption was turned off
		{
