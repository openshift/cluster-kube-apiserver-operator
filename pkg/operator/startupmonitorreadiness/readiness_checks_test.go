	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			doCheckAndValidate(t, func() (bool, string, string) {
				return newPodHasStateRunning(fakeKubeClient.CoreV1().Pods("openshift-kube-apiserver"))(context.TODO())
...
func TestNoOldRevisionPodExists(t *testing.T) {
	scenarios := []struct {
	...
	}{
		{
			...
			monitorRevision: 3,
			reason:          "InvalidPod",
			msg:             "the running Kube API is at unexpected revision 2, expected 3",
...
func TestNewRevisionPodExists(t *testing.T) {
	scenarios := []struct {
...

		{
			...
			reason: "InvalidPod",
			msg:    "the running Kube API is at unexpected revision 3, expected 4",
		},
	}
...
func TestGoodHealthzEndpoint(t *testing.T) {
	scenarios := []struct {
...

		{
			...
			msg: "failed while performing the check due to",
		},
		{
...
func TestGoodETCDEndpoint(t *testing.T) {
	scenarios := []struct {
		name        string
		healthy     bool
		reason      string
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			// act and validate
			doCheckAndValidate(t, func() (bool, string, string) { return goodETCDEndpoint(client, ts.URL)(context.TODO()) }, scenario.healthy, scenario.reason, scenario.msg)
		})
	}
