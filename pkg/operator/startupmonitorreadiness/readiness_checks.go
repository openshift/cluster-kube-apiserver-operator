...
// newPodHasStateRunning checks if kas pod is in PodRunning phase and has PodReady condition set to true
func newPodHasStateRunning(podClient corev1client.PodInterface) func(context.Context) (bool, string, string) {
...
func newRevisionPodExists(podClient corev1client.PodInterface, monitorRevision int) func(context.Context) (bool, string, string) {
	return func(ctx context.Context) (bool, string, string) {
		return checkRevision(ctx, podClient, monitorRevision, true)
	}
}
...
func noOldRevisionPodExists(podClient corev1client.PodInterface, monitorRevision int) func(context.Context) (bool, string, string) {
	return func(ctx context.Context) (bool, string, string) {
		return checkRevision(ctx, podClient, monitorRevision, false)
	}
}

// checkRevision checks if the kas pod is running at the expected revision
//
// strictMode controls whether a certain errors like: failing to get the pod or absence of the pod should be fatal
// it is useful when you want to avoid false positive - failing readyz check when the previous instance is still running
func checkRevision(ctx context.Context, podClient corev1client.PodInterface, monitorRevision int, strictMode bool) (bool, string, string) {
	apiServerPods, err := podClient.List(ctx, metav1.ListOptions{LabelSelector: "apiserver=true"})
	if err != nil {
	...
	if len(revisionString) == 0 {
		return false, "InvalidPod", fmt.Sprintf("empty revision label on %s pod", kasPod.Name)
	}
	revision, err := strconv.Atoi(revisionString)
	if err != nil || revision < 0 {
		return false, "InvalidPod", fmt.Sprintf("invalid revision label on pod %s: %q", kasPod.Name, revisionString)
	}
...
// goodETCDEndpoint performs an HTTP check against healthz/etcd endpoint
//  returns true, "", "", on HTTP 200
...
func goodETCDEndpoint(client *http.Client, rawURL string) func(context.Context) (bool, string, string) {
	return func(ctx context.Context) (bool, string, string) {
