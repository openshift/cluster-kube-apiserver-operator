...
// Only Vault KMS plugin is supported.
func AddKMSPluginToPodSpec(podSpec *corev1.PodSpec, featureGateAccessor featuregates.FeatureGateAccess, secretLister corev1listers.SecretLister) error {
	...
	// The first step is to read the encryption configuration secret generated by the state controller.
	// This secret contains the EncryptionConfiguration bytes and the configuration for the KMS plugin
	...
	// Parse the EncryptionConfiguration object. This object contains the KMS providers whose plugins should be deployed as sidecars
	...
	if !ok {
		return fmt.Errorf("unexpected type in encryption-config key: %T", obj)
	}

	// TODO: only Vault KMS plugin is supported for now, so *any* KMS configuration implies Vault
	...
	// Parse keyID from the UDS endpoint: "unix:///var/run/kmsplugin/kms-555.sock" -> "555"
	...
	// Now that the EncryptionConfiguration was parsed, read the provider config from the encryption-config secret
