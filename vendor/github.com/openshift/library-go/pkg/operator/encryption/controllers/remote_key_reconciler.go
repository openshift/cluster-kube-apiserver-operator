...
func reconcileRemoteKeyRotation(
	...
	if !secrets.IsBootstrapped(rk) {
		requeue, err := reconcileRemoteKeyBootstrap(ctx, secretClient, secretName, encryptedGRs, writeKey)
		return requeue, err
	}

	return reconcileRemoteKeyPromotion(ctx, secretClient, secretName, rk, convergedRemoteKeyID, clock)
}
...
func reconcileRemoteKeyPromotion(
	...
	convergedRemoteKey string,
	clock remoteKeyClock,
) (time.Duration, error) {
	if convergedRemoteKey == rk.TargetRemoteKeyID {
		return 0, clearRemoteKeyConvergence(ctx, secretClient, secretName, rk)
	}

	now := clock.Now()
	requeueAfter, promote, err := nextRemoteKeyPromotionAction(rk, convergedRemoteKey, now)
	if err != nil {
		return 0, err
	}
	...
	if promote {
		err = secrets.PatchRemoteKeyAnnotations(ctx, secretClient.Secrets("openshift-config-managed"), secretName, func(rk *secrets.RemoteKeyAnnotations) (bool, error) {
			if rk.TargetRemoteKeyID == convergedRemoteKey {
				return false, nil
			}
			if secrets.NeedsRemoteKeyMigration(*rk) {
				return false, nil
			}
			rk.TargetRemoteKeyID = convergedRemoteKey
			rk.ConvergedID = ""
	...
	err = secrets.PatchRemoteKeyAnnotations(ctx, secretClient.Secrets("openshift-config-managed"), secretName, func(rk *secrets.RemoteKeyAnnotations) (bool, error) {
		if rk.ConvergedID == convergedRemoteKey && !rk.ConvergedAt.IsZero() {
			return false, nil
		}
		rk.ConvergedID = convergedRemoteKey
		rk.ConvergedAt = now
		return true, nil
	})
