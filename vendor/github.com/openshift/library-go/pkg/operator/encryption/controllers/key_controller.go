...
func (c *keyController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	configError := c.checkAndCreateKeys(syncCtx.Queue(), syncCtx.Recorder())
...
func (c *keyController) checkAndCreateKeys(queue workqueue.RateLimitingInterface, recorder events.Recorder) error {
	...
	if createErr != nil {
		recorder.Warningf("EncryptionKeyCreateFailed", "Secret %q failed to create: %v", keySecret.Name, err)
		return createErr
	}

	recorder.Eventf("EncryptionKeyCreated", "Secret %q successfully created: %q", keySecret.Name, reasons)
