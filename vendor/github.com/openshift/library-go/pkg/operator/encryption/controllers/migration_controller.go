...
func (c *migrationController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	migratingResources, migrationError := c.migrateKeysIfNeededAndRevisionStable(syncCtx.Queue(), syncCtx.Recorder())
...
// TODO doc
func (c *migrationController) migrateKeysIfNeededAndRevisionStable(queue workqueue.RateLimitingInterface, recorder events.Recorder) (migratingResources []schema.GroupResource, err error) {
	...
	for _, gr := range grs {
		...
		if err := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			...
			_, _, updateErr := resourceapply.ApplySecret(c.secretClient, recorder, s)
			return updateErr
		}); err != nil {
			errs = append(errs, err)
			continue
