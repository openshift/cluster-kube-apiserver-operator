...
func (b *KMSPluginBuilder) applyHealthReporter(podSpec *corev1.PodSpec, sockets []string) error {
	...
	if b.healthReporter.name == "" || b.healthReporter.image == "" || b.healthReporter.subcommand == "" {
		return fmt.Errorf("health reporter name, image and subcommand are required when WithHealthReporter is used")
	}
...
func (h *healthReporter) Subcommand() string {
	// subcommand is the operator CLI subcommand registered via health.NewCommand
	// (see pkg/operator/encryption/kms/health/cmd.go). Also used as the init container name.
	return h.subcommand
}
