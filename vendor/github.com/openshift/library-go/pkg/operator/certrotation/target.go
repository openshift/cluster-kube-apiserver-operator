...
// KeyPairGeneratorConfigurable is optionally implemented by TargetCertCreator
// implementations to support configurable key algorithms via the
// ConfigurablePKI feature gate.
type KeyPairGeneratorConfigurable interface {
	SetKeyPairGenerator(g crypto.KeyPairGenerator)
}
...
func (c RotatedSelfSignedCertKeySecret) setTargetCertKeyPairSecret(targetCertKeyPairSecret *corev1.Secret, signer *crypto.CA) (*crypto.TLSCertificateConfig, error) {
	...
	if c.configurablePKIEnabled {
		...
		if kc, ok := c.CertCreator.(KeyPairGeneratorConfigurable); ok {
			kc.SetKeyPairGenerator(keyGen)
		}
...
func (c RotatedSelfSignedCertKeySecret) resolveKeyPairGenerator() (crypto.KeyPairGenerator, error) {
	return resolveKeyPairGeneratorWithFallback(c.pkiProfileProvider, c.CertificateType, c.CertificateName)
}
...
func (r *ClientRotation) SetKeyPairGenerator(g crypto.KeyPairGenerator) {
...
func (r *ClientRotation) NewCertificate(signer *crypto.CA, validity time.Duration) (*crypto.TLSCertificateConfig, error) {
	if r.configurablePKIEnabled {
		if r.keyGen != nil {
			return signer.NewClientCertificate(r.UserInfo, r.keyGen, crypto.WithLifetime(validity))
		}
		// nil keyGen means Unmanaged: fall through to legacy cert generation
	}
...
func (r *ServingRotation) SetKeyPairGenerator(g crypto.KeyPairGenerator) {
	r.configurablePKIEnabled = true
	r.keyGen = g
}
...
func (r *SignerRotation) SetKeyPairGenerator(g crypto.KeyPairGenerator) {
	r.configurablePKIEnabled = true
	r.keyGen = g
}
...
func (r *SignerRotation) NewCertificate(signer *crypto.CA, validity time.Duration) (*crypto.TLSCertificateConfig, error) {
	...
	if r.configurablePKIEnabled {
		if r.keyGen != nil {
			return crypto.NewSigningCertificate(signerName, r.keyGen,
