...
// ensureRequiredResourceExists makes sure that all non-optional resources are ready or it will return an error to trigger a requeue so that we try again.
func (c InstallerController) ensureRequiredResourceExists() error {
	missingSecrets := []string{}
	missingConfigs := []string{}

	for _, cm := range append(c.certConfigMaps, c.configMaps...) {
		if cm.Optional {
			continue
		}
	...
	for _, s := range append(c.certSecrets, c.secrets...) {
		if s.Optional {
			continue
		}
...
func (c InstallerController) sync() error {
	...
	err = c.ensureRequiredResourceExists()
