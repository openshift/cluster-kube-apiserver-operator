...
func (o *InstallOptions) copyContent(ctx context.Context) error {
	...
	// the key must contain pod.yaml to be considered
	for rawPodKey, rawPod := range podsConfigMap.Data {
		if !strings.Contains(rawPodKey, "pod.yaml") {
			continue
		}
		var podName string
		if rawPodKey == "pod.yaml" {
			// this is our main pod, assign the previous name for compatibility
			podName = o.PodConfigMapNamePrefix + ".yaml"
		}

		klog.Infof("Writing a pod under %q key \n%s", rawPodKey, rawPod)
		err := o.writePod([]byte(rawPod), podName, resourceDir)
		if err != nil {
			return err
		}
	}
...
func (o *InstallOptions) writePod(rawPodBytes []byte, podFileName, resourceDir string) error {
