...
func NewInstaller() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			ctx, cancel := context.WithTimeout(context.TODO(), time.Duration(o.TimeoutSeconds)*time.Second)
			defer cancel()
...
func (o *InstallOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&o.TimeoutSeconds, "timeout-seconds", 120*time.Second, "maximum time in seconds to wait for the copying to complete (default: 2m)")
}
...
func (o *InstallOptions) Validate() error {
	...
	if o.TimeoutSeconds == 0 {
		return fmt.Errorf("--timeout-seconds cannot be 0")
	}
...
func (o *InstallOptions) copyContent(ctx context.Context) error {
	...
	err = utilwait.PollImmediateUntil(200*time.Millisecond, func() (bool, error) {
		...
		if podData, exists := podConfigMap.Data["pod.yaml"]; !exists {
			return false, fmt.Errorf("required 'pod.yaml' key does not exists in configmap")
		} else {
	...
	o.NodeName, err = o.getPodNodeName([]byte(podContent))
	if err != nil {
...
func (o *InstallOptions) getPodNodeName(podBytes []byte) (string, error) {
	...
	nodeName, _, err := unstructured.NestedString(unstructuredPodObj.(*unstructured.Unstructured).Object, "spec.nodeName")
	return nodeName, err
}
