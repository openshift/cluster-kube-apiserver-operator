...
func (e *etcdWrapper) Get(ctx context.Context, key string, opts ...clientv3.OpOption) (*clientv3.GetResponse, error) {
	// The port-forward and etcd request go through the kube-apiserver, which on SNO
	// can be briefly unavailable during a revision rollout. Retry the whole build+get
	// while the caller's context is alive so a transient failure doesn't fail the test.
