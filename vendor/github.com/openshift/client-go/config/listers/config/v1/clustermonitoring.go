...
type ClusterMonitoringLister interface {
	...
	List(selector labels.Selector) (ret []*configv1.ClusterMonitoring, err error)
	// Get retrieves the ClusterMonitoring from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*configv1.ClusterMonitoring, error)
...
type clusterMonitoringLister struct {
	listers.ResourceIndexer[*configv1.ClusterMonitoring]
}
...
func NewClusterMonitoringLister(indexer cache.Indexer) ClusterMonitoringLister {
	return &clusterMonitoringLister{listers.New[*configv1.ClusterMonitoring](indexer, configv1.Resource("clustermonitoring"))}
}
