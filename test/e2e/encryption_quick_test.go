...
func TestEncryptionIntegration(tt *testing.T) {
	...
	require.NoError(t, err)
	kubeInformers := v1helpers.NewKubeInformersForNamespaces(kubeClient, operatorclient.GlobalMachineSpecifiedConfigNamespace)
	...
	waitForKeys := func(n int) {
		t.Helper()
		err := wait.PollImmediate(time.Second, wait.ForeverTestTimeout, func() (bool, error) {
			l, err := kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).List(metav1.ListOptions{LabelSelector: keySecretsLabel})
			if err != nil {
				return false, err
			}
			if len(l.Items) == n {
	...
	_, err = kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Patch(fmt.Sprintf("encryption-key-%s-5", component), types.MergePatchType, []byte(`{"metadata":{"annotations":{"encryption.apiserver.operator.openshift.io/migrated-timestamp":"2010-10-17T14:14:52+02:00"}}}`))
	require.NoError(t, err)
	waitForKeys(6)
	waitForConfigs(
	...
	_, err = kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Patch(fmt.Sprintf("encryption-key-%s-6", component), types.JSONPatchType, []byte(`[{"op":"remove","path":"/metadata/finalizers"}]`))
	require.NoError(t, err)
	err = kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Delete(fmt.Sprintf("encryption-key-%s-6", component), nil)
	require.NoError(t, err)
	err = wait.PollImmediate(time.Second, wait.ForeverTestTimeout, func() (bool, error) {
		_, err := kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Get(fmt.Sprintf("encryption-key-%s-7", component), metav1.GetOptions{})
	...
	_, err = kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Patch(fmt.Sprintf("encryption-config-%s", component), types.JSONPatchType, []byte(`[{"op":"remove","path":"/metadata/finalizers"}]`))
	require.NoError(t, err)
	err = kubeClient.CoreV1().Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace).Delete(fmt.Sprintf("encryption-config-%s", component), nil)
	require.NoError(t, err)
	waitForConfigs(
		"kubeapiservers.operator.openshift.io=aescbc:7,identity;kubeschedulers.operator.openshift.io=aescbc:7,identity",
	)
...
func NewInstantDeployer(t T, stopCh <-chan struct{}, kubeInformers v1helpers.KubeInformersForNamespaces, secretsClient corev1client.SecretsGetter,
	...
	return &lockStepDeployer{
		...
		configManagedSecretsClient: secretInterceptor{
			...
			SecretInterface: secretsClient.Secrets(operatorclient.GlobalMachineSpecifiedConfigNamespace),
...
func (d *lockStepDeployer) Secrets(namespace string) corev1client.SecretInterface {
	if namespace == operatorclient.GlobalMachineSpecifiedConfigNamespace {
