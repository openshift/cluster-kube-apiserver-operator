...
var _ = g.Describe("[Jira:kube-apiserver][sig-api-machinery][FeatureGate:EventTTL] Event TTL Configuration", func() {
	...
	g.BeforeAll(func() {
		...
		for _, featureGateDetails := range featureGate.Status.FeatureGates {
			for _, enabledFeature := range featureGateDetails.Enabled {
				if string(enabledFeature.Name) == "EventTTL" {
					foundFeature = true
					isEnabled = true
					g.By("EventTTL feature gate is already enabled")
					break
				}
			}
			if !foundFeature {
		...
		if !isEnabled {
			...
			g.By("EventTTL feature gate patch applied")
	...
	//testValues := []int32{5, 10}
	//for _, ttlMinutes := range testValues {
	ttl := int32(5)

	g.It(fmt.Sprintf("should configure and validate eventTTLMinutes=%dm [Timeout:60m][Operator][Serial]", ttl), ote.Informing(), func() {
		startTime := time.Now()
		...
		defer func() {
			...
			g.By("eventTTLMinutes configuration restored")
			...
			if originalEventTTL != 0 {
				...
				g.By("Kube-apiserver rollout completed after cleanup")
			} else {
...
func verifyEventTTLInConfigFile(ctx context.Context, kubeClient *kubernetes.Clientset, expectedMinutes int32) {
	...
	err := wait.PollUntilContextTimeout(ctx, 5*time.Second, 2*time.Minute, false, func(cxt context.Context) (bool, error) {
		...
		if hasCorrectValue {
			g.By(fmt.Sprintf("Found event-ttl=%s in config.yaml", expectedTTL))
			return true, nil
		}
